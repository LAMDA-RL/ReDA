...
def restore_pool_d4rl(replay_pool, name, adapt=True, maxlen=5, policy_hook=None,value_hook=None,device=None, dataset=None):
    import gym
    import d4rl
    if 'sac_data' in name:
    ...
    #data['rewards'] = torch.unsqueeze(data['rewards'], dim=1)
    data['terminals'] = np.expand_dims(data['terminals'], axis=1)
    #data['terminals'] = torch.unsqueeze(data['terminals'], dim=1)
    ...
    # for key in data:
    #     data[key] = torch.from_numpy(data[key]).cuda()
    max_traj_len = -1
    last_start = 0
    traj_num = 1
    traj_lens = []
    ...
    print('[ DEBUG ] : # of trajectories {}'.format(traj_num))
    print(ill_idxs)
    ...
    if adapt and policy_hook is not None:
        ...
        for i_ter in range(int(np.ceil(traj_num / traj_num_to_infer))):
            ...
            # states = torch.zeros((len(traj_lens_it), max_traj_len, data['observations'].shape[-1])).cuda()
            # actions = torch.zeros((len(traj_lens_it), max_traj_len, data['actions'].shape[-1])).cuda()
            # lst_actions = torch.zeros((len(traj_lens_it), max_traj_len, data['last_actions'].shape[-1])).cuda()
            ...
            # with torch.no_grad():
            #     states_tmp = torch.from_numpy(states).to(device)
            #     lst_actions_tmp = torch.from_numpy(lst_actions).to(device)
            #     policy_hidden_out = get_policy_hidden.get_hidden(states_tmp,\
            #                                                      lst_actions_tmp, torch.IntTensor(traj_lens_it))
            #     value_hidden_out = get_value_hidden.get_hidden(states_tmp,\
            #                                                    lst_actions_tmp, torch.IntTensor(traj_lens_it))
            #     policy_hidden_out2 = policy_hidden_out.cpu().detach().numpy()
            #     value_hidden_out2 = value_hidden_out.cpu().detach().numpy()
            #     del policy_hidden_out
            #     del value_hidden_out
            # policy_hidden = np.concatenate((np.zeros((len(traj_lens_it), 1, policy_hidden_out2.shape[-1])),
            #                                 policy_hidden_out2[:, :-1]), axis=-2)
            # value_hidden = np.concatenate((np.zeros((len(traj_lens_it), 1, value_hidden_out2.shape[-1])),
            #                                value_hidden_out2[:, :-1]), axis=-2)
            ...
            for ind, item in enumerate(traj_lens_it):
                if data['policy_hidden'] is None:
                    ...
                    # data['policy_hidden'] = torch.zeros((data['last_actions'].shape[0], policy_hidden.shape[-1])).cuda()
                    # data['value_hidden'] = torch.zeros((data['last_actions'].shape[0], value_hidden.shape[-1])).cuda()
...
def reset_hidden_state(replay_pool, name, maxlen=5, policy_hook=None, value_hook = None,device=None, fake_env=None):
    ...
    for i_ter in range(int(np.ceil(traj_num / traj_num_to_infer))):
        traj_lens_it = traj_lens[traj_num_to_infer * i_ter : min(traj_num_to_infer * (i_ter + 1), traj_num)]
        print('[ DEBUG ] : trajectories from {} to  {}'.format(traj_num_to_infer * i_ter,min(traj_num_to_infer * (i_ter + 1), traj_num))  )
        states = np.zeros((len(traj_lens_it), max_traj_len, data['observations'].shape[-1]), dtype=np.float32)
        actions = np.zeros((len(traj_lens_it), max_traj_len, data['actions'].shape[-1]), dtype=np.float32)
        lst_actions = np.zeros((len(traj_lens_it), max_traj_len, data['last_actions'].shape[-1]), dtype=np.float32)
        start_ind = last_start_ind
        ...
        #                                             torch.from_numpy(lst_actions).to(device), torch.IntTensor(traj_lens_it))
        with torch.no_grad():
            policy_hidden_out = torch.zeros((len(traj_lens_it), max_traj_len, get_policy.rnn_hidden_dim)).to(device)
            value_hidden_out = torch.zeros((len(traj_lens_it), max_traj_len, get_value.rnn_hidden_dim)).to(device)
...
def reset_hidden_meta(replay_pool, task, maxlen=5, policy_hook=None, value_hook = None,device=None, dataset=None):
    import gym
...
def restore_pool_meta(replay_pool, task, adapt=True, maxlen=5, policy_hook=None,value_hook=None,device=None, dataset=None):
    import gym
    ...
    data['rewards'] = np.expand_dims(np.squeeze(data['rewards']), 1)
    data['terminals'] = np.expand_dims(np.squeeze(data['terminals']), 1)
    data['last_actions'] = np.concatenate((np.zeros((1, data['actions'].shape[1]), dtype=np.float32), data['actions'][:-1, :]), axis=0).copy()
    data['first_step'] = np.zeros_like(data['terminals'])
    data['end_step'] = np.zeros_like(data['terminals'])
    data['valid'] = np.ones_like(data['terminals'])
    ...
    if adapt and policy_hook is not None:
        ...
        for i_ter in range(int(np.ceil(traj_num / traj_num_to_infer))):
            ...
            #                                             torch.from_numpy(lst_actions).to(device), torch.IntTensor(traj_lens_it))
            with torch.no_grad():
                policy_hidden_out = torch.zeros((len(traj_lens_it), max_traj_len, get_policy_hidden.rnn_hidden_dim if get_policy_hidden is not None else 128)).to(device)
                value_hidden_out = torch.zeros((len(traj_lens_it), max_traj_len, get_value_hidden.rnn_hidden_dim if get_policy_hidden is not None else 128)).to(device)
            max_len = max(traj_lens_it)
            policy_hidden = np.concatenate((np.zeros((len(traj_lens_it), 1, policy_hidden_out.shape[-1]), dtype=np.float32),
                                            policy_hidden_out[:, :-1].cpu().detach().numpy()), axis=-2)
            value_hidden = np.concatenate((np.zeros((len(traj_lens_it), 1, value_hidden_out.shape[-1]), dtype=np.float32),
