...
class GaussianActor(nn.Module, BasePolicy):
    def __init__(self, obs_dim, action_dim, hidden_size, hidden_layers):
        ...
        self.register_parameter('max_logstd', torch.nn.Parameter(torch.ones(action_dim) * 0.25, requires_grad=True))
        self.register_parameter('min_logstd', torch.nn.Parameter(torch.ones(action_dim) * -5, requires_grad=True))
