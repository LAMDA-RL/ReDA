...
class Transformer(nn.Module):
    def __init__(self, input_dim, embed_dim, heads, depth, output_dim):
        super().__init__()
        ...
        for i in range(depth):
            tblocks.append(
                TransformerBlock(emb=embed_dim, heads=heads, mask=False))

        self.tblocks = nn.Sequential(*tblocks)

        self.toprobs = nn.Linear(embed_dim, output_dim)

    def forward(self, tokens, hidden=None, mask=None, reduction='mean'):
        ...
        if reduction == 'mean':
            hidden = x.mean(dim=1, keepdim=True)
        elif reduction == 'sum':
            hidden = x.sum(dim=1, keepdim=True)
        else:
            hidden = x

        return hidden, hidden.squeeze(1)  # , tokens

    def zero_hidden(self, batch_size):
        return torch.zeros([1, batch_size, self.rnn_hidden_dim]).to(self.device)
